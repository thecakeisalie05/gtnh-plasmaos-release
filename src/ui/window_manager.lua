...
function WindowManager:_dirty(window)
  if self.damage then self.damage(window and {x = window.x, y = window.y, w = window.w, h = window.h}
...
function WindowManager:hit(x, y)
  for index = #self.order, 1, -1 do
    ...
    if window and window.state ~= "minimized" and x >= window.x and x < window.x + window.w
      ...
      if localY == 1 and localX == window.w then area = "close"
      elseif localY == 1 and localX == window.w - 2 then area = "maximize"
      elseif localY == 1 and localX == window.w - 5 then area = "minimize"
